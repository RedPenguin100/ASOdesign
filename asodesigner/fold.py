...
from asodesigner.consts import RISEARCH1_BINARY_PATH, TMP_PATH ,RISEARCH_1_PATH
...
def get_trigger_mfe_scores_by_risearch(trigger: str, name_to_sequence: Dict[str, str],
                                       ...
                                       minimum_score: int = 900, neighborhood: int = 0, parsing_type=None, target_file_cache=None,e1:int = 249 , e2:int = 100) -> str:
...
#
    ...
    if interaction_type == Interaction.DNA_RNA_NO_WOBBLE:
        ...
        m = 't04'
    elif     interaction_type == Interaction.MODIFIED: # read from csv_files in risearch dynamically 
        m = f'modified {e1} {e2}'
    else:
    ...
    try:
        ...
        result = subprocess.check_output(args, universal_newlines=True, text=True ,cwd=RISEARCH_1_PATH)
    finally:
        if target_file_cache is None:
            print()
